...
class ASSIT(nn.Module):
    ...
    def _split_regions(self, x):
        ...
        padded = F.pad(x, (0, 0, 0, pad_w, 0, pad_h))
...
class TransferModule(nn.Module):
    ...
    def forward(self, x):
        # x: [time, source, destination, channel]
        q = self.query(x)
        k = self.key(x)
        v = self.value(x)
        outputs = []
        for t in range(x.size(0)):
            ...
            if t > 0:
                ...
                scores = scores * weights[None, None, :]
            attention = F.softmax(scores, dim=-1)
            context = torch.einsum("ijk,kijc->ijc", attention, v[: t + 1])
...
class SatFormer(nn.Module):
    ...
    def forward(self, x, adjacency_matrices):
        ...
        for time_step in range(x.size(-1)):
            encoded.append(
                self._encode_time_step(
                    x[:, :, time_step],
                    adjacency_matrices[:, :, time_step],
                )
            )

        transferred = self._run_transfer(torch.stack(encoded, dim=0))
        decoded = []
        for time_step in range(transferred.size(0)):
            decoded.append(
                self._decode_time_step(
                    transferred[time_step],
                    adjacency_matrices[:, :, time_step],
                ).unsqueeze(-1)
            )
    ...
    def forward_time_window(self, x_window, adjacency_window, target_offset=-1):
        ...
        for time_step in range(x_window.size(-1)):
            encoded.append(
                self._encode_time_step(
                    x_window[:, :, time_step],
                    adjacency_window[:, :, time_step],
                )
            )

        transferred = self._run_transfer(torch.stack(encoded, dim=0))
        return self._decode_time_step(
            transferred[target_offset],
            adjacency_window[:, :, target_offset],
        )
