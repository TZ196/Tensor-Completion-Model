...
def build_timesnet_configs(args, input_dim, seq_len):
...
def make_time_marks(seq_len):
    steps = np.arange(seq_len, dtype="float32")
    marks = np.stack(
        [
            np.sin(2.0 * np.pi * steps / max(seq_len, 1)),
            np.cos(2.0 * np.pi * steps / max(seq_len, 1)),
...
def default_output_path(output_dir, observed_ratio, val_ratio, seed,
    ...
    name = "timesnet_random_observed%d_val%d_seed%d_norm_%s.json" % (
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run TimesNet tensor completion on satellite path traffic."
    )
    parser.add_argument(
        "--tensor-path",
