...
# Same visible rates as the previous missing-rate experiments:
# 1/3/5/7/10/20% visible = 99/97/95/93/90/80% missing.
VISIBLE_RATES=(${VISIBLE_RATES:-1 3 5 7 10 20})

# Six GT-MST variants, excluding M0 ID-MLP.
VARIANTS=(${VARIANTS:-M1 M2 M3 M4 M5 M6})

mkdir -p logs results splits checkpoints histories

timestamp="$(date +%Y%m%d_%H%M%S)"
master_log="logs/gt_mst_visible_rate_6_models_${timestamp}.log"
pid_file="logs/gt_mst_visible_rate_6_models.pid"

if [[ "${1:-}" != "--foreground" ]]; then
  nohup bash "$0" --foreground > "$master_log" 2>&1 &
  pid="$!"
  echo "$pid" > "$pid_file"
  echo "Started GT-MST 6-model visible-rate run in background."
  echo "PID: $pid"
...
echo "===== GT-MST 6-Model Visible-Rate Experiments ====="
...
for rate in "${VISIBLE_RATES[@]}"; do
  ...
  for variant in "${VARIANTS[@]}"; do
    name="vis${rate}_${variant}_gt_mst_6model_seed${SEED}"
    run_step "$name" \
...
echo "===== GT-MST 6-model visible-rate run finished at $(date) ====="
"$PYTHON_BIN" - <<'PY'
import json
import os

rates = [int(v) for v in os.environ.get("VISIBLE_RATES", "1 3 5 7 10 20").split()]
variants = os.environ.get("VARIANTS", "M1 M2 M3 M4 M5 M6").split()
seed = os.environ.get("SEED", "3")

names = {
    "M1": "Transformer",
    "M2": "Transformer+GraphToken",
    "M3": "M2+TextOnly",
    "M4": "M2+NumericOnly",
    "M5": "M2+TextNumeric",
...
print(f"{'Visible':>7s}  {'Variant':>7s}  {'Model':28s}  {'NMAE':>10s}  {'NRMSE':>10s}")
print("-" * 72)
for rate in rates:
    for variant in variants:
        path = f"results/vis{rate}_{variant}_gt_mst_6model_seed{seed}.json"
        label = names.get(variant, variant)
        if not os.path.exists(path):
            print(f"{rate:>6d}%  {variant:>7s}  {label:28s}  {'missing':>10s}  {'missing':>10s}")
            continue
        with open(path, encoding="utf-8") as f:
            payload = json.load(f)
        test = payload["test"]
        print(f"{rate:>6d}%  {variant:>7s}  {label:28s}  {test['nmae']:10.6f}  {test['nrmse']:10.6f}")
    print("-" * 72)
PY
