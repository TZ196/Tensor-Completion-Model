...
# 7%  checks text/numeric interaction stability.
# 10% checks whether numeric-gate keeps the previous numeric advantage.
# 20% checks whether V2 avoids hurting the high-observation regime.
VISIBLE_RATES=(${VISIBLE_RATES:-7 10 20})
VARIANTS=(${VARIANTS:-M2 M3 M4 M5 M6})

mkdir -p logs results splits checkpoints histories

timestamp="$(date +%Y%m%d_%H%M%S)"
master_log="logs/gt_mst_stable_v2_validation_${timestamp}.log"
pid_file="logs/gt_mst_stable_v2_validation.pid"

if [[ "${1:-}" != "--foreground" ]]; then
  nohup bash "$0" --foreground > "$master_log" 2>&1 &
  pid="$!"
  echo "$pid" > "$pid_file"
  echo "Started Stable GT-MST V2 validation in background."
  echo "PID: $pid"
...
echo "===== Stable GT-MST V2 Validation ====="
...
for rate in "${VISIBLE_RATES[@]}"; do
  ...
  for variant in "${VARIANTS[@]}"; do
    name="vis${rate}_${variant}_gt_mst_stable_v2_seed${SEED}"
    run_step "$name" \
...
echo "===== Stable GT-MST V2 validation finished at $(date) ====="
"$PYTHON_BIN" - <<'PY'
import json
import os
...
names = {
    "M2": "Transformer+ContextGraphToken",
    "M3": "M2+AdaptiveTextOnly",
    "M4": "M2+NumericOnly",
    "M5": "M2+TextWithNumericGate",
    "M6": "M5+TextAlign",
}

print(f"{'Visible':>7s}  {'Variant':>7s}  {'Model':30s}  {'NMAE':>10s}  {'NRMSE':>10s}")
print("-" * 76)
for rate in rates:
    for variant in variants:
        path = f"results/vis{rate}_{variant}_gt_mst_stable_v2_seed{seed}.json"
        label = names.get(variant, variant)
        if not os.path.exists(path):
            print(f"{rate:>6d}%  {variant:>7s}  {label:30s}  {'missing':>10s}  {'missing':>10s}")
            continue
        with open(path, encoding="utf-8") as f:
            payload = json.load(f)
        test = payload["test"]
        print(f"{rate:>6d}%  {variant:>7s}  {label:30s}  {test['nmae']:10.6f}  {test['nrmse']:10.6f}")
    print("-" * 76)
PY
