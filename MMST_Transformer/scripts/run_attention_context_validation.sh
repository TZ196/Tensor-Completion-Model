...
VARIANTS_RAW="${VARIANTS:-M2 M5 M6}"
...
master_log="logs/gt_mst_attention_context_${timestamp}.log"
pid_file="logs/gt_mst_attention_context.pid"
...
if [[ "${1:-}" != "--foreground" ]]; then
  ...
  pid="$!"
  echo "$pid" > "$pid_file"
  echo "Started GT-MST attention-context validation in background."
  echo "PID: $pid"
...
echo "===== GT-MST Attention Graph Context Validation ====="
...
for rate in "${VISIBLE_RATES[@]}"; do
  ...
  for variant in "${VARIANTS[@]}"; do
    name="vis${rate}_${variant}_gt_mst_attention_ctx_seed${SEED}"
    run_step "$name" \
...
echo "===== GT-MST attention-context validation finished at $(date) ====="
"$PYTHON_BIN" - <<'PY'
import json
import os

rates = [int(v) for v in os.environ.get("VISIBLE_RATES", "7 10 20").split()]
variants = os.environ.get("VARIANTS", "M2 M5 M6").split()
...
for rate in rates:
    for variant in variants:
        path = f"results/vis{rate}_{variant}_gt_mst_attention_ctx_seed{seed}.json"
