...
master_log="logs/gt_mst_static_gcn_m6_${timestamp}.log"
...
echo "===== Static-GCN GT-MST M6 Experiments ====="
...
for rate in "${VISIBLE_RATES[@]}"; do
  ...
  for variant in "${VARIANTS[@]}"; do
    name="vis${rate}_${variant}_gt_mst_static_gcn_seed${SEED}"
    run_step "$name" \
...
echo "===== Static-GCN GT-MST run finished at $(date) ====="
"$PYTHON_BIN" - <<'PY'
import json
import os

rates = [int(v) for v in os.environ.get("VISIBLE_RATES", "7 10 20").split()]
variants = os.environ.get("VARIANTS", "M6").split()
...
for rate in rates:
    for variant in variants:
        path = f"results/vis{rate}_{variant}_gt_mst_static_gcn_seed{seed}.json"
