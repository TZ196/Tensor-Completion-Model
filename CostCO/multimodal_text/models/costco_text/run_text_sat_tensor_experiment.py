...
from text_costco_model import (  # noqa: E402
    compile_mindtext_costco,
    create_mindtext_costco,
    evaluate_mindtext_costco,
)
...
def default_metrics_path(args, observed_ratio):
    name = (
        "mindtext_costco_%s_%s_observed%d_seed%d_rank%d_nc%d_"
        "text%d_lr%s_epoch%d.json"
    ) % (
        args.text_stage,
        args.text_ablation,
...
def main():
    ...
    fusion_name = (
        "costco_flow_plus_time_endo_text_and_global_exo_text"
        if is_global_context else "costco_flow_plus_endo_exo_text"
    )
    results = {
        "config": {
