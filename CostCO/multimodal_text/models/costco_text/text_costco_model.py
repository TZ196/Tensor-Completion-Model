...
from costco_model import (  # noqa: E402
    compile_costco,
    mae,
    nmae,
    nrmse,
...
def create_mindtext_costco(shape, endo_embeddings, exo_embeddings,
                           rank=50, nc=64, text_projection_dim=128,
                           text_stage="concat", alignment_projection_dim=128,
                           alignment_temperature=0.2, temporal_delta=2,
                           flow_text_loss_weight=0.0,
                           flow_text_reconstruction_weight=0.0,
                           condenser_alpha=1.0, condenser_epsilon=0.05,
    ...
    if flow_text_loss_weight > 0.0 or flow_text_reconstruction_weight > 0.0:
        ...
        )([flow_x, text_x, inputs[2]])

    fused = k.layers.Concatenate(name="flow_text_fusion")([flow_x, text_x])
...
def compile_mindtext_costco(model, lr=1e-4):
    return compile_costco(model, lr=lr)


def evaluate_mindtext_costco(model, indices, values, batch_size=1024,
