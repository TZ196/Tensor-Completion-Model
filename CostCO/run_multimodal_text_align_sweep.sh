...
master_log="logs/multimodal_text_align_sweep_${timestamp}.log"
pid_file="logs/multimodal_text_align_sweep.pid"

if [[ "${1:-}" != "--foreground" ]]; then
  nohup bash "$0" --foreground > "$master_log" 2>&1 &
  pid="$!"
  echo "$pid" > "$pid_file"
  echo "Started multimodal text-align sweep in background."
  echo "PID: $pid"
...
echo "===== Multimodal TextAlign Sweep ====="
...
for rate in "${VISIBLE_RATES[@]}"; do
  ...
  for text_align_ratio in $(align_ratios_for_rate "$rate"); do
    ...
    run_step "vis${rate}_gcn_costco_text_${tag}_seed${SEED}" \
      "$PYTHON_BIN" run_gcn_sat_tensor_experiment.py \
      "${common_args[@]}" \
      --observed-ratio "$ratio" \
      --split-path "$split_path" \
      --text-align-target-ratio "$text_align_ratio" \
      --metrics-path "results/vis${rate}_gcn_costco_text_${tag}_seed${SEED}.json"
  done
done

echo "===== Multimodal TextAlign sweep finished at $(date) ====="
"$PYTHON_BIN" - <<'PY'
import json
import os
...
for rate in rates:
    for align in ratio_map[rate]:
        ...
        path = f"results/vis{rate}_gcn_costco_text_{tag}_seed{seed}.json"
        if not os.path.exists(path):
            print(f"{rate:>6d}%  {align:8.3f}  {'missing':>10s}  {'missing':>10s}")
            continue
        with open(path) as f:
            payload = json.load(f)
        test = payload.get("test", payload)
        print(f"{rate:>6d}%  {align:8.3f}  {test['nmae']:10.6f}  {test['nrmse']:10.6f}")
PY
