...
master_log="logs/moderntcn_visible_rate_${timestamp}.log"
pid_file="logs/moderntcn_visible_rate.pid"

if [[ "${1:-}" != "--foreground" ]]; then
  nohup bash "$0" --foreground > "$master_log" 2>&1 &
  pid="$!"
  echo "$pid" > "$pid_file"
  echo "Started ModernTCN visible-rate experiments in background."
  echo "PID: $pid"
...
echo "===== ModernTCN Visible-Rate Experiments ====="
...
for rate in "${VISIBLE_RATES[@]}"; do
  ...
  metrics_path="results/vis${rate}_moderntcn_seed${SEED}.json"

  run_step "vis${rate}_moderntcn_seed${SEED}" \
...
echo "===== ModernTCN visible-rate experiments finished at $(date) ====="
"$PYTHON_BIN" - <<'PY'
import json
import os
...
print("-" * 47)
for rate in rates:
    path = f"results/vis{rate}_moderntcn_seed{seed}.json"
    if not os.path.exists(path):
        print(f"{rate:>6d}%  {'ModernTCN':12s}  {'missing':>10s}  {'missing':>10s}")
        continue
    with open(path) as f:
        payload = json.load(f)
    test = payload.get("test", payload)
    print(f"{rate:>6d}%  {'ModernTCN':12s}  {test['nmae']:10.6f}  {test['nrmse']:10.6f}")
PY
